...
from flaskblog.models import User_Accounts
...
class RegistrationForm(FlaskForm):
    ...
    def validate_username(self, username):
        user = User_Accounts.query.filter_by(username=username.data).first()
        if user:
            raise ValidationError('That username is taken. Please choose a different one.')

    def validate_email(self, email):
        user = User_Accounts.query.filter_by(email=email.data).first()
        if user:
...
class PostForm(FlaskForm):
    ...
    submit = SubmitField('Add This as a New Site')
